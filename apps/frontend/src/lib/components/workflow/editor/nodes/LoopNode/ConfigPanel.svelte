<script lang="ts">
	import type { LoopNodeData, LoopVariable, LoopBreakCondition } from './types';
	import { VARIABLE_TYPES, createDefaultLoopVariable, createDefaultBreakCondition } from './types';
	import { COMPARISON_OPERATORS, NO_VALUE_OPERATORS, type ComparisonOperator } from '../IfNode/types';
									...
									<div class="flex items-center gap-2 p-2">
										<div class="flex-1 min-w-0">
											<VariableSelect
												...
												onValueChange={(v: string | undefined) => updateBreakCondition(condition.id, 'variable', v ?? '')}
												placeholder="选择变量"
											/>
										</div>
										<Select.Root 
				...
				<div class="space-y-3">
					...
					<div class="flex items-center gap-4">
						...
						<Slider 
							type="single"
							value={maxIterations}
							onValueChange={(v) => updateField('maxIterations', v)}
							min={1}
							max={100}
							step={1}
							class="flex-1"
