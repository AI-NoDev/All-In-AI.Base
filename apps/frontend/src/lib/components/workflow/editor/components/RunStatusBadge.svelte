<script lang="ts">
	...
	import type { NodeRunStatus } from '$lib/components/workflow/types/index';

	/** 节点运行状态（兼容 'failed' 和 'error'）*/
	type RunStatusInput = NodeRunStatus | 'failed';
	...
	// 将 'failed' 映射为 'error'
	let normalizedStatus = $derived<NodeRunStatus>(status === 'failed' ? 'error' : status);
	let config = $derived(statusConfigs[normalizedStatus]);
