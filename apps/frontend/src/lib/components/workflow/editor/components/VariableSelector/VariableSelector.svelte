<script lang="ts">
	...
	let selectedVariable = $derived.by(() => {
		...
		for (const group of variableGroups) {
			const found = group.variables.find(v => v.path === value);
...
<Popover.Root bind:open>
	<Popover.Trigger {disabled}>
		<Button 
			variant="outline" 
			class="h-8 justify-start font-normal {className}"
			{disabled}
		>
			{#if selectedVariable}
				<Icon icon="mdi:code-braces" class="w-4 h-4 mr-2 text-primary shrink-0" />
				<span class="truncate">{selectedVariable.path}</span>
				<span class="ml-auto text-xs {getTypeColor(selectedVariable.type)}">{getTypeLabel(selectedVariable.type)}</span>
			{:else}
				<Icon icon="mdi:code-braces" class="w-4 h-4 mr-2 text-muted-foreground shrink-0" />
					...
					<div class="mb-2">
						...
						<div class="space-y-0.5">
							{#each group.variables as variable (variable.path)}
								<button
									class="w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-left hover:bg-muted transition-colors {value === variable.path ? 'bg-muted' : ''}"
