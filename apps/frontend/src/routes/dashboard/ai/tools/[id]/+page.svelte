<script lang="ts">
  ...
  async function loadTool() {
    try {
      const api = authStore.createApi(true);
      const res = await api.ai.getApiAiToolById({ id: toolId });
      if (res.data?.data) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      await api.ai.putApiAiToolById({ id: toolId }, { data });
      goto('/dashboard/ai/tools');
    } catch (err) {
...
{:else}
  <div class="flex flex-col gap-4 px-4 lg:px-6 pb-4">
    <div class="flex items-center gap-4">
      ...
      <h1 class="text-xl font-semibold">编辑工具</h1>
    </div>
