...
const app = new Elysia()
  .get('/schema', createActionHandler(jobLogGetSchema))
  .post('/query', createActionHandler(jobLogGetByPagination))
  .get('/:id', createActionHandler(jobLogGetByPk))
  .delete('/:id', createActionHandler(jobLogDeleteByPk));

const api = treaty(app);

describe('JobLog Actions', () => {
  describe('jobLogGetSchema', () => {
  ...
  describe.skipIf(!process.env.DATABASE_URL)('需要数据库', () => {
    it('jobLogGetByPagination', async () => {
    ...
    it('jobLogGetByPk - not found', async () => {
