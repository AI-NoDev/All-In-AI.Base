import type { WorkflowNode, WorkflowEdge, Variable, BaseNodeData, EnvironmentVariable } from '$lib/types/index.js';
...
function createWorkflowState<T extends BaseNodeData = BaseNodeData>() {
	...
	return {
		...
		createNodeFromPicker(template: PendingNodeTemplate) {
			...
			// 新节点的输入引脚默认在 top=28 的位置
			const TARGET_HANDLE_OFFSET = 28;
		...
		autoResizeLoopNode(loopNodeId: string) {
			...
			// 计算子节点边界（假设每个子节点宽度约 240px，高度约 100px）
			...
			// 计算需要的最小尺寸
			const minWidth = Math.max(500, maxRight + PADDING);
			const minHeight = Math.max(350, maxBottom + PADDING + HEADER_HEIGHT);
			
			// 更新循环节点尺寸
			const currentWidth = loopNode.width ?? 500;
