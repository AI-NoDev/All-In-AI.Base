<script lang="ts">
	import type { KnowledgeNodeData, KnowledgeBaseRef, MetadataFilterMode } from './types.js';
	...
	// 添加知识库
	function addKnowledgeBase(kbId: string) {
		const kb = MOCK_KNOWLEDGE_BASES.find(k => k.id === kbId);
		if (!kb) return;
		
		// 检查是否已添加
		if (knowledgeBases.some(k => k.id === kbId)) return;
	...
	let availableKnowledgeBases = $derived(
		MOCK_KNOWLEDGE_BASES.filter(kb => !knowledgeBases.some(k => k.id === kb.id))
	);
									...
									{#if availableKnowledgeBases.length > 0}
										<div class="space-y-1">
											{#each availableKnowledgeBases as kb (kb.id)}
												...
												>
													<Icon icon={kb.icon ?? 'mdi:database'} class="w-4 h-4 text-muted-foreground" />
													<span>{kb.name}</span>
												</button>
											{/each}
										</div>
					...
					{#if knowledgeBases.length > 0}
						<div class="space-y-2">
							{#each knowledgeBases as kb (kb.id)}
								{@const kbInfo = MOCK_KNOWLEDGE_BASES.find(k => k.id === kb.id)}
								<div class="border border-border rounded-lg">
									<div class="flex items-center justify-between px-3 py-2">
										<div class="flex items-center gap-2">
											<Icon icon={kbInfo?.icon ?? 'mdi:database'} class="w-4 h-4 text-purple-500" />
