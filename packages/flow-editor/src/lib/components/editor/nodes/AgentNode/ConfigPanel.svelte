<script lang="ts">
	import type { AgentNodeData, AgentInfo } from './types.js';
	import { MOCK_AGENTS, BUILTIN_OUTPUT_VARIABLES } from './types.js';
	...
	// 当前选中的 Agent
	let selectedAgent = $derived(MOCK_AGENTS.find(a => a.id === agentId));
	...
	function handleAgentChange(value: string | undefined) {
		if (!value) {
			...
			updateField('agentAvatar', undefined);
			updateField('agentColor', undefined);
			updateField('agentInputSchema', undefined);
			updateField('agentOutputSchema', undefined);
			return;
		}

		const agent = MOCK_AGENTS.find(a => a.id === value);
		if (agent) {
			...
			<div class="space-y-4">
				...
				<div class="space-y-2">
					<div class="flex items-center gap-1">
						<span class="text-xs font-medium">智能体</span>
							...
							{#if selectedAgent}
								<div class="flex items-center gap-2">
									<div 
										...
										style="background-color: {selectedAgent.color ?? '#6b7280'}"
									>
