<script lang="ts">
  ...
  import IconStop from '@iconify-svelte/tdesign/stop';
  import * as Tooltip from '@qiyu-allinai/ui/components/tooltip/index.js';
  import type { OutputNodeData, JsonSchemaProperty, EdgesGetter } from '../../types.js';
  ...
  let inputPins = $derived.by(() => {
    ...
    // 检查是否是完整的 JSON Schema 对象（带 type: 'object'）
    const schemaObj = schema as Record<string, unknown>;
    if (schemaObj.type === 'object' && schemaObj.properties) {
      return generatePins(schemaObj.properties as Record<string, JsonSchemaProperty>);
    }
    // 直接是 properties 对象
    return generatePins(schema as Record<string, JsonSchemaProperty>);
  });

  // 布局常量（与 StartNode 保持一致）
...
<div class="min-w-[180px] rounded-lg shadow-md text-primary-foreground relative bg-primary">
  ...
  {#each inputPins as pin, i}
    <Handle
      ...
      style="top: {getHandleTop(i)}px; left: 0px; width: {HANDLE_SIZE}px; height: {HANDLE_SIZE}px; background: {getHandleColor(pin.type)}; border: none;"
      {isConnectable}
    />
  {/each}
</div>
