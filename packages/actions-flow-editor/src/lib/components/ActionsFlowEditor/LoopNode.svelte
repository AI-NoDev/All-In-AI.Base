      ...
      {#each internalOutputPins as pin}
        <div class="flex flex-col items-center relative pb-2">
          ...
          <Handle
            ...
            style="position: absolute; bottom: -6px; left: 50%; transform: translateX(-50%); width: 8px; height: 8px; background: {getHandleColor(pin.type)}; border: 2px solid hsl(var(--primary-foreground)); border-radius: 50%;"
            {isConnectable}
          />
        </div>
      {/each}
