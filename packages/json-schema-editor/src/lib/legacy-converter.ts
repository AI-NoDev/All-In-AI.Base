...
import type { SchemaType, Field, RootSchema } from './legacy-types.js';
...
/** 导出为标准 JSON Schema */
export function legacyToJsonSchema(root: RootSchema): Record<string, unknown> {
  const properties: Record<string, unknown> = {};
  const required: string[] = [];
...
export function legacyFromJsonSchema(json: Record<string, unknown>): RootSchema {
  const properties = json.properties as Record<string, Record<string, unknown>> | undefined;
  const required = json.required as string[] | undefined;
  const fields: Field[] = [];
